...
class PyPass(object):
    ...
    def _reset(self):
        """
        Reset list of used characters. User when generating multiple passwords
        """
        self._special = []
        self._lower = []
        self._upper = []
...
if __name__ == '__main__':
    try:
        ...
        if arg == '-h':
            print 'Usage:'
            print 'python *.py (one 8-char long password)'
            print 'python *.py 12 (one 12-char long password)'
            print 'python *.py 12 10 (ten 12-char long password)'
            sys.exit(0)
