...
def update_stats():
    sys.stdout.write('\r|%d\t\t|%d\t\t|%d\t\t|%d\t\t|%s.' % (len(found), int(host_scanned / (time.time() - start_time)), threading.activeCount()-1, '.'.join(current_host.split('.')[0:3])))
    sys.stdout.flush()
...
def brute_force(host, timeout, s_object):
    global found
    global current_host
...
def main():
    ...
    global start_time
    if (len(sys.argv) < 8):
        print '[-] Usage: python %s [START_IP] [END_IP] [USERNAME-LIST] [PASSWORD-LIST] [OUTPUT-FILE] [THREADS] [TIMEOUT]' %sys.argv[0]
        sys.exit()
    ...
    threads = []
    se =threading.BoundedSemaphore(value=int(sys.argv[6]))
    ips = (int_to_ip(i) for i in xrange(ip_to_int(sys.argv[1], ip_to_int(sys.argv[2]))))
    print 'Scanning IP...\nFound\t\tHost/s\t\tThreads\t\tCurrent'
    start_time = time.time()
    for ip in ips:
        se.acquire()
        thread = threading.Thread(target=brute_force, args=(ip, float(sys.argv[7]), se))
        thread.start()
