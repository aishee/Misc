...
def load_config_file(config_file):
    with open(config_file) as f:
        file_content = "[root]\n"
        file_content += f.read()
    content_str_fp = StringIO(file_content)
...
class Email(object):
    ...
    def get_attached_files(self):
        """
        return list of tupe (filename, file_conent, Email object)
        """
        files = []
...
class FileMessage(object):
    """
    this class is used to handle the email.message. Message type attached files
    
    """
    def __init__(self, file_message, mail_message):
        self._file_message = file_messages
...
def reply(recipient):
    ...
    text = """ Dont't to nasty thing to me! """
...
def test_smtp():
    smtp_server, imap_server, username, password = get_user_info()
    text = """
    Don't do nasty thing to me!
    """
    repcipients = [""]
    msg = MIMEText(text, 'plain', 'utf-8')
    msg['Subject'] = Header("Hi , hello you!", 'utf-8')
    msg['From'] = ":)"
...
def test_vba():
    ...
    for the_file in os.listdir(path):
        ...
        with open(the_file, 'rb') as f:
            ...
            print_the_file + " : is_suspicious() = " + str(result)

def main():
    parser = argparse.ArgumentParser(
        description = "Parse atteched files in mails in a mailbox" + "and identify suspicious mails.\n" + "two modes\n" + "\t1. scan mode\n" + "\t2. monitor mode\n"
    )
    ...
    if mode == 'scan':
        ...
        print "Mode need tobe 'scan' or 'monitor' or 'combo'."
